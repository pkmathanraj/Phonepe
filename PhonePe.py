...
def clone():
    c=git.Git("data").clone("https://github.com/PhonePe/pulse")
...
# clone() 
# find a way to check for the updated content from git before downloading the repo. 
# If updated content is available then have a code to update the downloaded content as this one throws error as the 
# folder to download already exists ans is not empty dir.

#create_mysqlschema()
#filepaths = read_dir()
#extract_data(filepaths)
# all the above functions need to be executed only once
# especially data extraction and insertion as duplicates will be inserted into the table
